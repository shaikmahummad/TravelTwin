...
export function DigitalTwinScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, "DigitalTwin">) {
  ...
  return <ScrollView contentContainerStyle={styles.page}>
    ...
    {(twin.events || []).length ? (twin.events || []).slice(0, 3).map((event, index) => <AppCard key={`${event.name}-${index}`}><Text style={styles.event}>{event.event_type || "Cultural event"}</Text><Text style={styles.place}>{event.name || "Local heritage event"}</Text><Text style={styles.muted}>{event.location || "Heritage district"} · {event.price ? `₹${event.price}` : "Free"}</Text></AppCard>) : <AppCard><Text style={styles.muted}>No events published nearby right now.</Text></AppCard>}
